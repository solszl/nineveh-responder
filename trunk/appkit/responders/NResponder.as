...
package appkit.responders 
{
	...
	//**************************************************
		 ...
		 * 						So when you will dispatch the NResponder Action you can use the constant
		 * 						instead of writing the name of action.
		 * 						
		 ...
		 * 						How any listener can't be added to respond 0 times. So the 0 represent
		 * 						the infinity replies.
		 * 
		 * @param	delay		The delay time, in seconds, between two consecutives dispatchs.
		 * 						During the delay time the NResponder Action will not activate the listener.
		 * 						<br/>
		 * 						For example, if the delay time set for the mouse click on a button
		 * 						is of 2 seconds and this button is clicked, the listener only will respond
		 * 						to a new click after 2 seconds.
		 * 
		 ...
		 * 						<br/>
		 * 						For example an order of <code>n</code> is dispatched first than an order
		 * 						of <code>n+1</code>. If two or more listeners share the same priority, they 
		 * 						are dispatched in the order in which they were added. The default is 0. 
		 * 
		 ...
		 * 						<br/>
		 * 						You can access it during the execution of the listener by calling the
		 * 						propertie <code>currentTarget</code> of the <code>NResponder Class</code>.
		 * 
	...
	//**********************************************************************************************************
		 ...
		 * 						The methods are addEventListener() and removeEventListener().
		 * 						Not is necessary to this target to explicit implements de IEventDispatcher.
		 * 						This target will be used to perform the native Event API of Adobe® Flash®.
		 * 
		 * @param	action		The identifier of the Action. This action will be used to dispatch the Action.
		 * 						<br/>
		 * 						It is advisable to define a constant for each action of action.
		 ...
		 * <p/>
		 * 
		 * The <code>remove()</code> method can be used in three ways:
		 * 
		 * <ul>
		 * 
		 * <li>Remove a listener from a NResponder Action.
		 * To do this way, you need specify an action and a listener.
		 * For example: <code>myNResponder.remove('aAction', someFunction);</code></li>
		 * 
		 * <li>Remove entire a NResponder Action.
		 * To do this way, just specify the action and no listener.
		 * For example: <code>myNResponder.remove('aAction');</code></li>
		 * 
		 * <li>Remove all NResponder Actions from NResponder Chain.
		 * To do this way, just call the method with no parameter.
		 * For example: <code>myNResponder.remove();</code></li>
		 * 
		 * </ul>
		 * 
		 ...
		 * <p/>
		 * 
		 * The <code>has()</code> method can be used in two ways:
		 * 
		 * <ul>
		 * 
		 * <li>Check if a listener, with a specific action, is in NResponder Chain.
		 * To do this way, you need specify an action and a listener.
		 * For example: <code>myNResponder.has('aAction', someFunction);</code></li>
		 * 
		 * <li>Check if a specific action, with any listener, is in NResponder Chain.
		 * To do this way, you just need specify an action.
		 * For example: <code>myNResponder.has('aAction');</code></li>
		 * 
		 * </ul>
		 * 
		 * @param	action		The identifier of the Action.
		 ...
		 * effects, like replies or delays.
		 * 
		 * @langversion ActionScript 3.0
		 * @playerversion Flash 9.0
		 */
		 ...
		 * 
		 * @return				If the listener is performed return <code>true</code>, otherwise if this NResponder are in the
		 * delay time return <code>false</code>.
		 * 
		 * @langversion ActionScript 3.0
		 * @playerversion Flash 9.0
		 */
